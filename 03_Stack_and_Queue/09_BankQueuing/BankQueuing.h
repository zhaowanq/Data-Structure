...
}EventType;								//事件类型，0代表到达事件，1至4代表4个窗口的离开事件
typedef struct							//事件结点元素 
{
...
#include "../../02_LinearList/04_SinglyLinkedList/SinglyLinkedList.c"		/*02 线性表*/
typedef struct
{
	int ArrivedTime;					//客户到达时间 
	int Duration;						//办理事务所需的时间 
	int Count;							//记录来到每个队列的客户是第几个 
}QElemType_L;							//队列的数据元素类型 
#include "../../03_Stack_and_Queue/07_LinkQueue/LinkQueue.c"					/*03 栈和队列*/
