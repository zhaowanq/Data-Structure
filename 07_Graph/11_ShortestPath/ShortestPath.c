...
#include "ShortestPath.h" 							/* 07_Graph */
...
{
	// 若P1[v][w]为TRUE，则w是从v0到v当前求得的最短路径上的顶点。
	...
	for(i=2; i<=G1.vexnum; i++)							// 其余G1.vexnum-1个顶点
	{
		...
		{
			if(!final[w] && min<INFINITY && G1.arcs[v][w].adj<INFINITY && (min + G1.arcs[v][w].adj < D1[w]))	//修改D1[w]和P1[w],w属于V-S 
			{
...
{// 请自行debug,理解其输出过程,此代码有小问题,不要深究
	int i, j, k, tmp;
	int count[MAX_VERTEX_NUM+1];		// 每条最短路径中顶点个数
	int order[MAX_VERTEX_NUM+1];		// 先后顺序
		
	for(i=1; i<=G1.vexnum; i++)			// 初始化count和order数组
	{
		order[i] = i;
		count[i] = 0;
		for(j=1; j<=G1.vexnum; j++)
	...
	for(i=1; i<=G1.vexnum; i++)						
	{
		...
		{
			if(count[j]<count[i])							// 将count数组按照顶点个数递增的顺序存放
			{
				tmp = count[i];
				count[i] = count[j];
				count[j] = tmp;
				
				for(k=1; k<=G1.vexnum; k++)			// 构成最短路径时，顶点的先后顺序
				{
					tmp = P1[k][i];
					P1[k][i] = P1[k][j];
					P1[k][j] = tmp;
		...
		if(v0!=i)
		{
			...
			else
			{
				...
				if(v0!=i)
				{
					...
					do
					{
						tmp[0]++;							// tmp[0]存放顶点个数,tmp[1],tmp[2],tmp[3]...存放该路径上的顶点，且按照逆序存放各个顶点
						tmp[tmp[0]] = k;
						k = path1[k];			
					}while(path1[k]!=-1);			
				}
...
{
	// 若P2[v][w][u]为TRUE,则u是从v到w当前求得最短路径上的顶点

	int i, v, w, u;
	
	for(v=1; v<=G2.vexnum; v++)						// 各对结点之间初始已知路径及距离 
	{
...
void Floyd(MGraph G2, PathMatrix path2[][MAX_VERTEX_NUM+1], ShortPathTable dist2[][MAX_VERTEX_NUM+1])
{
	...
	// 输出各对顶点之间路径,输出的不是完整的路径,子问题中的路径没有显示地展现出来。要输出完整路径,尝试用递归实现。
	for(i=1; i<=G2.vexnum; i++)
	{
		for(j=1; j<=G2.vexnum; j++)
		{
